...
# ── S26 마무리 — 1×4 스토리 + 필독 3칸 ──
s = blank()
content_header(s, "마무리", "**회귀 extrap** — 오늘 정리")

rect(s, Inches(0.45), Inches(1.12), Inches(12.43), Inches(0.68), NAVY, TEAL, radius=0.05)
hero = s.shapes.add_textbox(Inches(0.65), Inches(1.18), Inches(12.0), Inches(0.58))
hero_tf = add_text(
    hero,
...
_sh = Inches(0.92)
_sy = Inches(1.92)
for i, (title, line, accent) in enumerate(_story):
    x = Inches(0.45 + i * (_sw + _sg))
    card_text(s, x, _sy, _sw, _sh, title, [line], accent, dense=True)
    if i < 3:
        arr = s.shapes.add_textbox(x + _sw + Inches(0.01), Inches(2.28), Inches(0.10), Inches(0.22))
        add_text(arr, "›", size=13, bold=True, color=MUTED, align=PP_ALIGN.CENTER)

_lbl = s.shapes.add_textbox(Inches(0.45), Inches(3.02), Inches(2.5), Inches(0.24))
...
for i, (role, cite, desc, key, accent) in enumerate(_trio):
    x = Inches(0.45 + i * (_tw + _tg))
    card_text(s, x, _ty, _tw, _th, f"{role} · {cite}", [desc], accent, dense=True)
    pdf_btn(s, key, x + (_tw - Inches(0.92)) / 2, _ty + Inches(0.82), w=Inches(0.92), h=Inches(0.28), label="PDF")
...
takeaway(s, "가정 맞추기 · **밖에서 시험** · UQ/기권",
         "Xu · Runje · Bartley", y=Inches(6.32))
